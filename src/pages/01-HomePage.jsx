...
export function HomePage({setSearchCompany,searchCompany}) {

    return (
        <>
            <MainHeader setSearchCompany={setSearchCompany} searchCompany={searchCompany} />

            {/* {guest === null && <MainIndex />} */}
            <MainGuestIndex setSearchCompany={setSearchCompany} />
...
function CompanyListComponent({ companyName, logo, address, about, id,setSearchCompany }) {
    const navigate = useNavigate();
    function handleClick(e){        
        setSearchCompany(e.target.name)
        navigate(`/search`)
    }
    return (
        <button type="button" name={companyName} className="list-group-item list-group-item-action bg-bg-black d-flex align-items-center justify-content-center w-50" onClick={handleClick}>{logo}-Şirket Adı:{companyName}-Adres:{address}</button>
    )
}

function MainGuestIndex({setSearchCompany}) {
        ...
        <main className="d-flex flex-column justify-content-center align-items-center">
            ...
            {guest !== null && <section className="companies text-center p-2 w-100 ">
                ...
                <div className="companies-div list-group l-w-50 d-flex justify-content-center align-items-center">
                    {companyList.length !== 0 && companyList.map(company => <CompanyListComponent setSearchCompany={setSearchCompany} key={company.companyName} {...company} />)}
